...
class SecureAPIManager {
  ...
  async saveAPIKey(
    ...
    try {
      ...
      // Sauvegarder dans Keychain pour les clés critiques
      if (this.isCriticalProvider(provider)) {
        await Keychain.setInternetCredentials(
          `camprompt-${provider}`,
          provider,
          key,
      ...
      if (!keyData) {
        ...
        if (this.isCriticalProvider(provider)) {
          try {
            ...
            logger.warn(`Keychain non disponible pour ${provider}`);
          }
        }
