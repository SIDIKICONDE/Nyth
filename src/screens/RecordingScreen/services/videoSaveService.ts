...
export class VideoSaveService {
  ...
  static async saveRecording({
    ...
    try {
      ...
      // Get file info for iOS
      const fileInfo = await this.getFileInfo(savedVideoPath);
      
      logger.info("Video saved to local storage", {
        recordingId,
        savedVideoPath,
        fileExists: await RNFS.exists(savedVideoPath),
        fileInfo,
      });
  ...
  /**
   * Ensure file path has proper prefix
   */
