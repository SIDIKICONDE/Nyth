...
export function useErrorRecovery(options: ErrorRecoveryOptions = {}) {
  ...
  const attemptRecovery = useCallback(
    (error: Error, analysis: { severity: string; recoverable: boolean }) => {
      ...
      // Calculer le délai de récupération (backoff exponentiel avec limite)
      const delay = Math.min(retryDelay * Math.pow(2, state.retryCount), 10000); // Max 10 secondes
      ...
      recoveryTimeoutRef.current = setTimeout(() => {
        ...
        logger.info(`Récupération automatique effectuée (essai ${state.retryCount + 1}/${maxRetries})`);
        onRecovered?.();
      }, delay);
    },
