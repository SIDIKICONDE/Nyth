...
const SubscriptionManagementScreen: React.FC = () => {
  ...
  const handleOpenCustomerPortal = () => {
    if (isStripeConfigured() && subscription && subscription.status === "active") {
      setShowCustomerPortal(true);
    } else {
      Alert.alert(
        t("subscription.portalUnavailable.title", "Portail indisponible"),
        t("subscription.portalUnavailable.message", "Le portail client n'est disponible que pour les abonnements Stripe actifs.")
      );
    }
  };
    ...
    return (
      <View style={[
        ...
        <View style={tw`flex-row gap-3`}>
          ...
          {isStripeConfigured() && subscription.status === "active" && (
            <TouchableOpacity
              style={[
                tw`flex-1 py-3 px-4 rounded-lg border`,
                { 
            ...
            >
              <Text style={[
                ...
                {t("subscription.managePortal", "Gérer")}
              </Text>
            </TouchableOpacity>
          )}
        </View>
