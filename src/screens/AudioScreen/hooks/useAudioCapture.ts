...
export function useAudioCapture(options: UseAudioCaptureOptions = {}) {
  ...
  const [captureState, setCaptureState] = useState<CaptureState>('uninitialized');
  ...
  const initialize = useCallback(async () => {
    try {
      ...
      if (success) {
        ...
        if (onStateChange) {
          NativeAudioCaptureModule.setStateChangeCallback((oldState, newState) => {
  ...
  const startRecording = useCallback(async (filePath: string, recordingOptions?: RecordingOptions) => {
    try {
      if (!isInitialized) {
      ...
      const success = NativeAudioCaptureModule.startRecording(filePath, recordingOptions);
  ...
  const analyzeAudioFile = useCallback(async (filePath: string) => {
    try {
      logger.debug('📊 Analyse du fichier audio:', filePath);
      const analysis = await NativeAudioCaptureModule.analyzeAudioFile(filePath);
