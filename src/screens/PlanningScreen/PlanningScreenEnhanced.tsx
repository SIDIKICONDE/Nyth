...
import { Alert, View, StatusBar, ScrollView, ActivityIndicator, Pressable, StyleSheet } from "react-native";
...
import { FilterModal } from "./components/enhanced/FilterModal";
import { SearchModal } from "./components/enhanced/SearchModal";
...
const ENHANCED_PLANNING_TABS: Tab[] = [
  ...PLANNING_TABS,
  { key: "teams", title: "planning.tabs.teams", icon: "people-outline" },
];
  ...
  return (
    <View style={[statsStyles.container, { backgroundColor: currentTheme.colors.surface }]}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        ...
        <View style={statsStyles.statItem}>
          <Icon name="time-outline" size={20} color={currentTheme.colors.secondary} />
...
export const PlanningScreenEnhanced: React.FC = () => {
  ...
  const [isOfflineMode, _setIsOfflineMode] = useState(false);
  ...
  const handleExport = useCallback(async (_format: 'json' | 'csv' | 'pdf') => {
    try {
      ...
      <View style={enhancedStyles.headerContainer}>
        ...
        <View style={enhancedStyles.actionBar}>
          ...
          >
            ...
            {Object.values(modalState.filters).some(
              (f) => f !== 'all' && (Array.isArray(f) ? f.length > 0 : f !== undefined)
