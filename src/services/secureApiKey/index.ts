...
export class SecureApiKeyService {
  ...
  static async authenticateUser(
    ...
    return AuthService.authenticateUser(reason);
  }
    ...
    try {
      ...
      if (requireAuth) {
        const authenticated = await this.authenticateUser(
          `Sauvegarder la clé ${provider}`
        );
        if (!authenticated) {
          logger.warn("Authentification refusée");
          return false;
        }
      }
