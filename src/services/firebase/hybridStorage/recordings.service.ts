...
export class RecordingsService {
  async saveRecording(
    ...
    try {
      ...
      const sourcePath = toLocalPath(videoUri);
      ...
      if (thumbnailUri && thumbnailFileName) {
        ...
        const thumbnailSource = toLocalPath(thumbnailUri);
        try {
      ...
      // 3. Sauvegarder uniquement les métadonnées dans Firestore (meilleur effort)
      ...
      try {
        ...
        // Ne pas bloquer le flux si le cloud échoue : la vidéo est déjà sauvegardée localement
        logger.warn("Échec d'écriture des métadonnées Firestore (continuation en local)", cloudError);

        // Notifier l'utilisateur de l'erreur de synchronisation
        const notificationService = FirestoreErrorNotificationService.getInstance();
        await notificationService.notifyError(
          FirestoreErrorType.METADATA_SAVE_FAILED,
          cloudError instanceof Error ? cloudError : new Error(String(cloudError)),
          {
            showAlert: true,
            persistNotification: true,
            customMessage: `Les métadonnées de votre enregistrement "${scriptTitle || 'Sans titre'}" n'ont pas pu être synchronisées avec le cloud, mais la vidéo est sauvegardée localement.`
          }
        );
      }
    ...
    try {
      ...
      for (const doc of snapshot.docs) {
        ...
        try {
          ...
          // Ignorer les erreurs de stat pour les fichiers inexistants
        }
      }
