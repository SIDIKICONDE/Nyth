...
export class PerformanceMonitor {
  ...
  // IDs pour les intervalles et timeouts
  private memoryMonitoringTimeout: NodeJS.Timeout | null = null;
  private interactionMonitoringInterval: NodeJS.Timeout | null = null;
  private periodicReportingInterval: NodeJS.Timeout | null = null;
  ...
  private startMemoryMonitoring(): void {
    const measureMemory = () => {
      ...
      this.memoryMonitoringTimeout = setTimeout(measureMemory, 5000);
    };
  ...
  /**
   * Nettoie toutes les ressources (intervalles, timeouts, etc.)
   */
  private cleanup(): void {
    ...
    // Arrêter le timeout de monitoring mémoire
    if (this.memoryMonitoringTimeout !== null) {
      clearTimeout(this.memoryMonitoringTimeout);
      this.memoryMonitoringTimeout = null;
    }

    // Arrêter l'interval de monitoring des interactions
    if (this.interactionMonitoringInterval !== null) {
      clearInterval(this.interactionMonitoringInterval);
      this.interactionMonitoringInterval = null;
    }

    // Arrêter l'interval de reporting périodique
    if (this.periodicReportingInterval !== null) {
