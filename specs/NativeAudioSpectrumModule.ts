...
export type SpectrumErrorType =
  (typeof SpectrumError)[keyof typeof SpectrumError];
...
export type SpectrumStateType =
  (typeof SpectrumState)[keyof typeof SpectrumState];
...
class NativeAudioSpectrumModuleWrapper implements Spec {
  ...
  processAudioBufferStereo(
    ...
    if (
      !audioBufferL ||
      !audioBufferR ||
      audioBufferL.length === 0 ||
      audioBufferR.length === 0 ||
  ...
  validateConfig(config: SpectrumConfig): boolean {
    ...
    if (
      ...
      return false;
    if (config.fftSize && (config.fftSize & (config.fftSize - 1)) !== 0)
      return false;
    if (config.minFreq && config.maxFreq && config.minFreq >= config.maxFreq)
      return false;
    return true;
  }
  ...
  private _errorCallback: ((error: number, message: string) => void) | null =
    null;
  private _stateCallback:
    | ((oldState: number, newState: number) => void)
    | null = null;
  ...
  setErrorCallback(
    callback: (error: number, message: string) => void,
  ): boolean {
