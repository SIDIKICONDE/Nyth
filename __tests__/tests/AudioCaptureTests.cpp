...
#include "../shared/Audio/capture/components/AudioCapture.hpp"
#include "../shared/Audio/capture/components/AudioCaptureException.hpp"
...
#include "../shared/Audio/capture/components/AudioFileWriter.hpp"
#include <thread>
#include <chrono>
#include <random>
#include <numeric>
...
TEST_F(AudioSIMDTest, SIMDRMSMatchesScalar) {
    ...
    float rmsSIMD = AudioNR::MathUtils::MathUtilsSIMDExtension::calculateRMSSIMD(sine.data(), sine.size());
...
TEST_F(AudioSIMDTest, SIMDPeakMatchesScalar) {
    ...
    float peakSIMD = AudioNR::MathUtils::MathUtilsSIMDExtension::calculatePeakSIMD(noise.data(), noise.size());
...
TEST_F(AudioSIMDTest, SIMDClippingDetectionMatchesScalar) {
    ...
    // Note: countClippedSamples_Optimized n'est pas dans MathUtilsSIMDExtension,
    // nous allons le tester avec les fonctions de base.
    float peakSIMD = AudioNR::MathUtils::MathUtilsSIMDExtension::calculatePeakSIMD(data.data(), data.size());
    size_t clippedSIMD = 0;
    for(size_t i = 0; i < data.size(); ++i) {
        if (std::abs(data[i]) >= 0.99f) { // Utiliser un seuil standard
            clippedSIMD++;
        }
    }
