...
jsi::Value NativeAudioCaptureModule::isSilent(jsi::Runtime& rt, double threshold) {
    ...
    if (threshold < AudioLimits::MIN_THRESHOLD || threshold > AudioLimits::MAX_THRESHOLD) {
        throw jsi::JSError(rt, "Threshold must be between " + 
                         std::to_string(AudioLimits::MIN_THRESHOLD) + 
                         " and " + std::to_string(AudioLimits::MAX_THRESHOLD));
    }
...
jsi::Value NativeAudioCaptureModule::setStateChangeCallback(jsi::Runtime& rt, const jsi::Function& callback) {
    ...
    try {
        ...
        if (capture_) {
            capture_->setStateChangeCallback([this](Audio::capture::CaptureState oldState,
...
jsi::Value NativeAudioCaptureModule::setAnalysisCallback(jsi::Runtime& rt, const jsi::Function& callback,
    ...
    try {
        ...
        if (intervalMs < AudioLimits::MIN_ANALYSIS_INTERVAL_MS || 
            intervalMs > AudioLimits::MAX_ANALYSIS_INTERVAL_MS) {
            throw jsi::JSError(rt, "Analysis interval must be between " + 
                             std::to_string(AudioLimits::MIN_ANALYSIS_INTERVAL_MS) + 
                             " and " + std::to_string(AudioLimits::MAX_ANALYSIS_INTERVAL_MS) + " ms");
        }
...
Audio::capture::AudioCaptureConfig NativeAudioCaptureModule::parseConfigSafe(jsi::Runtime& rt,
    ...
    try {
        ...
        if (jsConfig.hasProperty(rt, "sampleRate")) {
            ...
            config.sampleRate = static_cast<int>(
                JSIValidator::validateNumberInRange(rt, prop, "sampleRate", 
                    AudioLimits::MIN_SAMPLE_RATE, AudioLimits::MAX_SAMPLE_RATE)
            );
        }
        ...
        if (jsConfig.hasProperty(rt, "channelCount")) {
            ...
            config.channelCount = static_cast<int>(
                JSIValidator::validateNumberInRange(rt, prop, "channelCount",
                    AudioLimits::MIN_CHANNELS, AudioLimits::MAX_CHANNELS)
            );
        }
        ...
        if (jsConfig.hasProperty(rt, "bitsPerSample")) {
            ...
            int bits = static_cast<int>(
                JSIValidator::validateNumberInRange(rt, prop, "bitsPerSample",
                    AudioLimits::MIN_BITS_PER_SAMPLE, AudioLimits::MAX_BITS_PER_SAMPLE)
            );
        ...
        if (jsConfig.hasProperty(rt, "bufferSizeFrames")) {
            ...
            config.bufferSizeFrames = static_cast<int>(
                JSIValidator::validateNumberInRange(rt, prop, "bufferSizeFrames",
                    AudioLimits::MIN_BUFFER_SIZE_FRAMES, AudioLimits::MAX_BUFFER_SIZE_FRAMES)
            );
        }
