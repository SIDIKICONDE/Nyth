...
NythCaptureState NythCapture_GetState(void) {
    std::lock_guard<std::mutex> lock(g_globalMutex);
    if (!g_captureInstance) return CAPTURE_STATE_UNINITIALIZED;
...
jsi::Value NativeAudioCaptureModule::startRecording(jsi::Runtime& rt, const jsi::String& filePath, const jsi::Object& options) {
    std::lock_guard<std::mutex> lock(captureMutex_);
...
void NativeAudioCaptureModule::handleAudioData(const float* data, size_t frameCount, int channels) {
    if (!jsCallbacks_.audioDataCallback) return;
    ...
    invokeJSCallback("audioData", [this, dataCopy = std::move(dataCopy), frameCount, channels](jsi::Runtime& rt) mutable {
        ...
        // Appeler le callback
        jsCallbacks_.audioDataCallback->call(rt, float32Array, 
...
void NativeAudioCaptureModule::handleError(const std::string& error) {
    if (!jsCallbacks_.errorCallback) return;
...
void NativeAudioCaptureModule::handleStateChange(Audio::capture::CaptureState oldState, Audio::capture::CaptureState newState) {
    ...
    invokeJSCallback("stateChange", [this, oldState, newState](jsi::Runtime& rt) {
        ...
        jsCallbacks_.stateChangeCallback->call(rt, 
                                              jsi::String::createFromUtf8(rt, oldStateStr),
...
jsi::Value NativeAudioCaptureModule::install(jsi::Runtime& rt, std::shared_ptr<CallInvoker> jsInvoker) {
    ...
    registerMethod("start", 0, [](auto* m, auto& rt, auto*, auto) { 
        return m->start(rt); 
    });
    
    registerMethod("stop", 0, [](auto* m, auto& rt, auto*, auto) { 
        return m->stop(rt); 
    });
    
    registerMethod("pause", 0, [](auto* m, auto& rt, auto*, auto) { 
        return m->pause(rt); 
    });
    
    registerMethod("resume", 0, [](auto* m, auto& rt, auto*, auto) { 
        return m->resume(rt); 
    });
