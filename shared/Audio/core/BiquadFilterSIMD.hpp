...
public:
     ...
     */
    void processStereoAVX2(const float* inputL, const float* inputR,
     ...
     */
    void processStereoNEON(const float* inputL, const float* inputR,
                          float* outputL, float* outputR, size_t numSamples) {
        const float32x4_t a0_vec = vdupq_n_f32(static_cast<float>(m_a0));
        const float32x4_t a1_vec = vdupq_n_f32(static_cast<float>(m_a1));
        const float32x4_t a2_vec = vdupq_n_f32(static_cast<float>(m_a2));
...
public:
    explicit VectorizedBuffer(size_t size) 
    ...
    VectorizedBuffer(VectorizedBuffer&& other) noexcept 
