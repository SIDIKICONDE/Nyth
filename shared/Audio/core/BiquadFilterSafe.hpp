...
public:
     ...
     */
    AudioError processStereoSafe(const float* inputL, const float* inputR,
     ...
     */
    AudioError setCoeffcientsSafe(double a0, double a1, double a2, 
...
public:
    SafeAudioBuffer(T* data, size_t size) 
    ...
    AudioError validate() const noexcept {
        if (!m_data) return AudioError::NULL_POINTER;
        if (m_size == 0) return AudioError::INVALID_SIZE;
