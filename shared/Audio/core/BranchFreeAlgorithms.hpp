...
inline float hardClip(float x) noexcept {
    ...
    union { float f; uint32_t i; } one = { 1.0f };
    union { float f; uint32_t i; } neg_one = { -1.0f };
    ...
    // Apply masks
    val.i = (val.i & ~(gt_one | lt_neg_one)) | 
            (one.i & gt_one) | 
            (neg_one.i & lt_neg_one);
...
public:
    EnvelopeFollower(float attackTime, float releaseTime, float sampleRate)
        : m_envelope(0.0f) {
