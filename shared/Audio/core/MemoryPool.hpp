...
public:
    explicit LockFreeMemoryPool(size_t poolSize = 1024)
        : m_poolSize(poolSize), m_allocated(0) {
    ...
    void deallocate(T* ptr) noexcept {
        ...
        for (size_t i = 0; i < m_poolSize; ++i) {
            ...
            if (m_freeList[i].compare_exchange_strong(expected, node,
                                                      std::memory_order_release)) {
    ...
    void reset() noexcept {
        ...
        for (size_t i = 0; i < m_poolSize; ++i) {
            m_freeList[i].store(reinterpret_cast<Node*>(&m_memory[i]), 
                              std::memory_order_release);
        }
    }
...
public:
    explicit RingBufferPool(size_t bufferSize = 4096, size_t numBuffers = 32)
        ...
        m_memory = static_cast<float*>(std::aligned_alloc(64, 
                                       bufferSize * numBuffers * sizeof(float)));
        if (!m_memory) {
            throw std::bad_alloc();
        }
    ...
    size_t getNumBuffers() const noexcept { return m_numBuffers; }
...
public:
    explicit StackAllocatorScope(StackAllocator& allocator)
        : m_allocator(allocator), m_mark(allocator.mark()) {}
...
class ObjectPool {
public:
    explicit ObjectPool(size_t poolSize = 64)
        : m_poolSize(poolSize) {
...
public:
    PooledObject(ObjectPool<T>& pool)
        : m_pool(pool), m_object(pool.acquire()) {}
