...
public:
    ...
    bool isOk() const noexcept { return m_error == AudioError::OK; }
    bool hasError() const noexcept { return m_error != AudioError::OK; }
    ...
    // Access error
    AudioError error() const noexcept { return m_error; }
...
public:
    ...
    static AudioError validateBuffer(const T* buffer, size_t size) noexcept {
        if (!buffer && size > 0) return AudioError::NULL_POINTER;
        if (size == 0) return AudioError::INVALID_SIZE;
        if (size > 1024*1024*100) return AudioError::BUFFER_TOO_LARGE; // 100MB limit
    ...
    static AudioError validateRange(T value, T min, T max) noexcept {
        if (value < min || value > max) return AudioError::OUT_OF_RANGE;
        return AudioError::OK;
    }
    ...
    static AudioError validateFinite(T value) noexcept {
        if (std::isnan(value)) return AudioError::NAN_DETECTED;
        if (std::isinf(value)) return AudioError::INF_DETECTED;
        return AudioError::OK;
    }
};
...
#define AUDIO_RETURN_IF_ERROR(expr) \
    do { \
        ...
        if (_err != AudioError::OK) return _err; \
    } while(0)

#define AUDIO_RETURN_RESULT_IF_ERROR(expr) \
    do { \
        AudioError _err = (expr); \
        if (_err != AudioError::OK) return AudioResult<void>(_err); \
    } while(0)
