...
public:
    ...
    inline float dbToLinear(float db) const noexcept {
        ...
        if (db >= MAX_DB) return MAX_LINEAR;
    ...
    inline float linearToDb(float linear) const noexcept {
        ...
        if (linear <= MIN_LINEAR) return MIN_DB;
        if (linear >= MAX_LINEAR) return MAX_DB;
...
public:
    ...
    inline float dbToGain(float gainDb) const noexcept {
        if (gainDb <= MIN_GAIN_DB) return 0.0f;
        if (gainDb >= MAX_GAIN_DB) return m_gainTable[GAIN_TABLE_SIZE - 1];
...
namespace FastMath {
    ...
    inline float ultrafast_db_to_linear(float db) noexcept {
        ...
        if (db >= 0.0f) return 1.0f + db * 0.115f; // Linear approximation near 0dB
