...
#include "utils/AudioCaptureUtils.hpp"
#include <atomic>
...
struct AudioFileWriterConfig {
    ...
    int channelCount = Constants::DEFAULT_CHANNEL_COUNT;
    int bitsPerSample = Constants::DEFAULT_BITS_PER_SAMPLE;
    bool appendMode = false;  // Si true, ajoute à un fichier existant
    size_t bufferSize = Constants::MAX_BUFFER_SIZE; // Taille du buffer d'écriture
};
...
public:
    ...
    AudioFileWriterConfig getConfig() const {
        return config_;
    }
...
private:
    ...
    struct WAVHeader {
        ...
        // Format chunk
        char fmtId[4] = {'f', 'm', 't', ' '};
        uint32_t fmtSize = Constants::WAV_FORMAT_CHUNK_SIZE;
        uint16_t audioFormat = Constants::WAV_FORMAT_PCM; // PCM
        uint16_t numChannels = 1;
        uint32_t sampleRate = Constants::DEFAULT_SAMPLE_RATE;
        uint32_t byteRate = 0;
        uint16_t blockAlign = 0;
        uint16_t bitsPerSample = Constants::DEFAULT_BITS_PER_SAMPLE;
...
public:
    ...
    bool initialize(std::shared_ptr<AudioCapture> capture, const AudioFileWriterConfig& writerConfig);
...
private:
    ...
    float durationLimit_ = Constants::DEFAULT_DURATION_VALUE;
...
public:
    ...
    struct SplitConfig {
        ...
        float splitDuration = Constants::DEFAULT_DURATION_LIMIT;          // Durée en secondes pour BY_DURATION
        size_t splitSize = Constants::DEFAULT_SPLIT_SIZE_MB; // Taille en octets pour BY_SIZE
        float silenceThreshold = Constants::SILENCE_DETECTION_THRESHOLD;      // Seuil pour BY_SILENCE
        float silenceDuration = Constants::DEFAULT_SILENCE_DURATION;         // Durée de silence pour déclencher la division

        std::string filePattern = "recording_{index}.wav"; // Pattern pour les noms de fichiers
        int startIndex = Constants::DEFAULT_FILE_START_INDEX;
    };
    ...
    bool initialize(std::shared_ptr<AudioCapture> capture, const SplitConfig& config,
    ...
    void setFileSplitCallback(FileSplitCallback callback) {
        fileSplitCallback_ = callback;
    }
...
inline float AudioFileWriter::getDurationSeconds() const {
    if (config_.sampleRate == Constants::NULL_DATA_CHECK)
        return Constants::DEFAULT_METRICS_VALUE;
    return static_cast<float>(framesWritten_.load()) / config_.sampleRate;
}

inline float AudioRecorder::getRecordingDuration() const {
    if (!writer_.isOpen() || writerConfig_.sampleRate == Constants::NULL_DATA_CHECK)
        return Constants::DEFAULT_METRICS_VALUE;
