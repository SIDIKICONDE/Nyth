...
bool AudioCaptureIOS::setupAudioUnit() {
  ...
  status = AudioUnitSetProperty(audioUnit_, kAudioOutputUnitProperty_EnableIO,
                                ...
                                Constants::IOS_AUDIO_INPUT_BUS, // input bus
  ...
  status = AudioUnitSetProperty(audioUnit_, kAudioOutputUnitProperty_EnableIO,
                                ...
                                Constants::IOS_AUDIO_OUTPUT_BUS, // output bus
...
void AudioCaptureIOS::CircularBuffer::write(const float *data, size_t frames) {
  ...
  for (size_t i = 0; i < frames; ++i) {
    ...
    writePos = (writePos + Constants::IOS_BUFFER_POSITION_INCREMENT) % size;
  }
}
...
size_t AudioCaptureIOS::CircularBuffer::read(float *data, size_t maxFrames) {
  ...
  while (framesRead < maxFrames && readPos != writePos) {
    ...
    readPos = (readPos + Constants::IOS_BUFFER_POSITION_INCREMENT) % size;
    framesRead++;
  }
  return framesRead;
}
...
void AudioCaptureIOS::CircularBuffer::clear() {
  std::lock_guard<std::mutex> lock(mutex);
  writePos = Constants::IOS_BUFFER_POSITION_RESET;
  readPos = Constants::IOS_BUFFER_POSITION_RESET;
}
...
std::vector<AudioDeviceInfo> AudioCaptureIOS::getAvailableDevices() const {
  ...
  for (AVAudioSessionPortDescription *port in inputs) {
    ...
    device.maxChannels = Constants::IOS_MAX_CHANNELS_DEFAULT; // Valeur par défaut
    device.supportedSampleRates = {Constants::DEFAULT_SAMPLE_RATE,
                                   Constants::SAMPLE_RATE_48KHZ}; // Taux d'échantillonnage courants
