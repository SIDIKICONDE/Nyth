...
AudioMetrics::AudioMetrics()
    : enabled_(true)
    , windowSize_(1024)
    , updateInterval_(100) // 100ms par défaut
...
void AudioMetrics::process(const float* data, size_t frameCount, int channelCount) {
    ...
    current.rmsDb = 20.0f * std::log10(std::max(current.rms, 1e-10f));
    ...
    current.peakDb = 20.0f * std::log10(std::max(current.peak, 1e-10f));
    ...
    current.truePeakDb = 20.0f * std::log10(std::max(current.truePeak, 1e-10f));
    
    // Crest factor
    if (current.rms > 0) {
        current.crestFactor = current.peak / current.rms;
        current.crestFactorDb = 20.0f * std::log10(current.crestFactor);
    }
...
void AudioMetrics::updateAverages(const MetricsData& current) {
    ...
    averageMetrics_.rmsDb = 20.0f * std::log10(std::max(averageMetrics_.rms, 1e-10f));
    
    averageMetrics_.peak = averageMetrics_.peak * (1 - alpha) + current.peak * alpha;
    averageMetrics_.peakDb = 20.0f * std::log10(std::max(averageMetrics_.peak, 1e-10f));
...
void AudioMetrics::updateHistory(const MetricsData& current) {
    const size_t maxHistorySize = 1000; // Keep last 1000 samples
...
float AudioMetrics::calculateLUFS(const float* data, size_t frameCount, int channelCount) {
    ...
    // Convert to LUFS
    float lufs = -0.691f + 10.0f * std::log10(std::max(kWeightedPower, 1e-10f));
...
float AudioMetrics::calculateDynamicRange(const float* data, size_t sampleCount) {
    ...
    const size_t windowSize = std::min(size_t(256), sampleCount);
    ...
    size_t idx95 = static_cast<size_t>(windowRMS.size() * 0.95);
    size_t idx10 = static_cast<size_t>(windowRMS.size() * 0.10);
    
    float loud = windowRMS[idx95];
    float quiet = windowRMS[idx10];
    
    if (quiet > 0) {
        return 20.0f * std::log10(loud / quiet);
    }
...
float AudioMetrics::estimateSpectralCentroid(const float* data, size_t sampleCount) {
    ...
    // Assuming sample rate of 44100 Hz (should be passed as parameter)
    const float sampleRate = 44100.0f;
    float estimatedFreq = zcr * sampleRate / 2.0f;
...
void PerformanceMonitor::endFrame(size_t samplesProcessed) {
    ...
    float latencyMs = duration / 1000.0f;
    ...
    float expectedDuration = (samplesProcessed / 44100.0f) * 1000.0f; // ms
    float cpuUsage = (latencyMs / expectedDuration) * 100.0f;
    
    stats_.cpuUsage = cpuUsage;
    cpuHistory_.push_back(cpuUsage);
    if (cpuHistory_.size() > 1000) {
        cpuHistory_.erase(cpuHistory_.begin());
    }
    
    // Check for dropouts
    if (latencyMs > expectedDuration * 0.8f) { // 80% threshold
        stats_.dropouts++;
    }
}
