...
public:
    ...
    AudioException(AudioErrorType type, const std::string& message, 
    ...
    AudioErrorType getType() const { return type_; }
...
public:
    DeviceNotFoundException(const std::string& deviceId)
        : AudioException(AudioErrorType::DeviceNotFound, 
...
public:
    PermissionDeniedException()
        : AudioException(AudioErrorType::PermissionDenied, 
...
public:
    BufferOverflowException(size_t bufferSize, size_t requestedSize)
        : AudioException(AudioErrorType::BufferOverflow,
                        "Buffer overflow: requested " + std::to_string(requestedSize) +
...
public:
    ...
    static void validateBitsPerSample(int bits) {
        if (bits != 8 && bits != 16 && bits != 24 && bits != 32) {
            throw InvalidConfigurationException(
                "Invalid bits per sample: " + std::to_string(bits) + 
    ...
    static void validateBufferSize(int bufferSize) {
        if (bufferSize < 64 || bufferSize > 8192) {
            throw InvalidConfigurationException(
                "Invalid buffer size: " + std::to_string(bufferSize) + 
    ...
    static void validateConfig(const AudioCaptureConfig& config) {
        ...
        if (config.numBuffers < 2 || config.numBuffers > 10) {
            throw InvalidConfigurationException(
                "Number of buffers must be between 2 and 10");
        }
    }
};
