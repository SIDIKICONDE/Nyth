...
BenchmarkStats calculateStats(const std::vector<double>& times, double baseline = 0) {
    ...
    stats.median = sorted.size() % 2 == 0 ? (sorted[mid - 1] + sorted[mid]) / 2.0 : sorted[mid];
...
void printResults(const std::string& name, const BenchmarkStats& stats) {
    std::cout << std::left << std::setw(30) << name << " | " << std::fixed << std::setprecision(2)
              << "Mean: " << std::setw(8) << stats.mean << " ns | "
              << "Median: " << std::setw(8) << stats.median << " ns | "
              << "StdDev: " << std::setw(8) << stats.stddev << " ns | "
              << "Speedup: " << std::setw(6) << stats.speedup << "x" << std::endl;
}
...
void benchmarkSIMD() {
    ...
    std::cout << "SIMD provides " << std::setprecision(1) << (scalarStats.mean / simdStats.mean) << "x speedup"
              << std::endl;
}
...
void benchmarkCombined() {
    ...
    std::cout << "\n🚀 Total speedup: " << std::setprecision(1) << (baselineStats.mean / optimizedStats.mean) << "x"
              << std::endl;
}
