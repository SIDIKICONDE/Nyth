...
NativeAudioNoiseModule::NativeAudioNoiseModule(std::shared_ptr<CallInvoker> jsInvoker) : TurboModule() {
    ...
    config_ = Nyth::Audio::NoiseConfig();
}
...
void NativeAudioNoiseModule::initializeManagers() {
    ...
    noiseManager_->setStatisticsCallback(
        [this](const Nyth::Audio::NoiseStatistics& stats) { this->onStatisticsUpdate(stats); });
...
void NativeAudioNoiseModule::onStatisticsUpdate(const Nyth::Audio::NoiseStatistics& stats) {
    if (callbackManager_ && runtimeValid_.load()) {
        try {
