...
void WienerFilter::processSpectrum(const std::vector<float>& realIn,
...
void WienerFilter::processMagnitudePhase(const std::vector<float>& magnitude,
...
void TwoStepNoiseReduction::process(const std::vector<float>& magnitude,
                                    const std::vector<float>& phase,
                                    std::vector<float>& outputMagnitude) {
...
void TwoStepNoiseReduction::estimateResidualNoise(const std::vector<float>& original,
    ...
    for (size_t k = 0; k < original.size(); ++k) {
        ...
        if (residual > cfg_.residualThreshold * residualNoise_[k]) {
            residualNoise_[k] = cfg_.residualSmoothing * residualNoise_[k] + 
                                (1.0f - cfg_.residualSmoothing) * std::abs(residual);
        }
    }
}

void TwoStepNoiseReduction::getStepGains(std::vector<float>& step1Gains,
