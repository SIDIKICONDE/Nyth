...
NoiseReducer::NoiseReducer(uint32_t sampleRate, int numChannels) : sampleRate_(sampleRate), channels_(numChannels) {
    ...
    if (channels_ < MIN_CHANNELS)
        channels_ = MIN_CHANNELS;
    if (channels_ > MAX_CHANNELS)
        channels_ = MAX_CHANNELS;
...
void NoiseReducer::setSampleRate(uint32_t sampleRate) {
    ...
    if (sampleRate_ == sampleRate)
        return; // Optimization: no change needed
...
void NoiseReducer::setConfig(const NoiseReducerConfig& cfg) {
    ...
    if (cfg.thresholdDb > MAX_THRESHOLD_DB || cfg.thresholdDb < MIN_THRESHOLD_DB) {
        throw std::invalid_argument("Threshold must be between " + std::to_string(MIN_THRESHOLD_DB) + " and " +
                                    std::to_string(MAX_THRESHOLD_DB) + " dB");
    }
    if (cfg.ratio < MIN_RATIO || cfg.ratio > MAX_RATIO) {
        throw std::invalid_argument("Ratio must be between " + std::to_string(MIN_RATIO) + " and " +
                                    std::to_string(MAX_RATIO));
    }
    if (cfg.floorDb > MAX_FLOOR_DB || cfg.floorDb < MIN_FLOOR_DB) {
        throw std::invalid_argument("Floor must be between " + std::to_string(MIN_FLOOR_DB) + " and " +
                                    std::to_string(MAX_FLOOR_DB) + " dB");
    }
    if (cfg.attackMs < MIN_ATTACK_MS || cfg.attackMs > MAX_ATTACK_MS) {
        throw std::invalid_argument("Attack time must be between " + std::to_string(MIN_ATTACK_MS) + " and " +
                                    std::to_string(MAX_ATTACK_MS) + " ms");
    }
    if (cfg.releaseMs < MIN_RELEASE_MS || cfg.releaseMs > MAX_RELEASE_MS) {
        throw std::invalid_argument("Release time must be between " + std::to_string(MIN_RELEASE_MS) + " and " +
                                    std::to_string(MAX_RELEASE_MS) + " ms");
    }
    if (cfg.highPassHz < MIN_HIGHPASS_HZ || cfg.highPassHz > MAX_HIGHPASS_HZ) {
        throw std::invalid_argument("High-pass frequency must be between " + std::to_string(MIN_HIGHPASS_HZ) + " and " +
                                    std::to_string(MAX_HIGHPASS_HZ) + " Hz");
    }
...
void NoiseReducer::processMono(const float* input, float* output, size_t numSamples) {
    ...
    if (numSamples == ZERO_SAMPLES_CHECK)
        return;
...
void NoiseReducer::processChannel(const float* in, float* out, size_t n, ChannelState& st) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // Envelope
        if (ax > st.env)
            st.env = attackCoeffEnv_ * st.env + (UNITY_GAIN - attackCoeffEnv_) * ax;
        else
            st.env = releaseCoeffEnv_ * st.env + (UNITY_GAIN - releaseCoeffEnv_) * ax;
        ...
        if (st.env < threshLin_) {
            ...
            // Apply floor limit
            if (gTarget < floorLin_)
                gTarget = floorLin_;
        }

        // Smooth gain (avoid pumping)
        if (gTarget > st.gain)
            st.gain = attackCoeffGain_ * st.gain + (UNITY_GAIN - attackCoeffGain_) * gTarget;
        else
            st.gain = releaseCoeffGain_ * st.gain + (UNITY_GAIN - releaseCoeffGain_) * gTarget;
